{{-- Tabla de evidencias compartida. Espera: $items (colección de TicketMedia),
     $avatarTone (tone del avatar, ej: 'rose'|'primary'), $fallbackInitial, más $fmtDate/$fileNameFor del scope padre. --}}
    ...
    <table class="ticket-show__table">
        ...
        <tbody>
            @foreach ($items as $media)
                <tr>
                    <td class="is-strong max-w-[140px] truncate" title="{{ $vm->fileNameFor($media) }}">
                        {{ $vm->fileNameFor($media) }}
                    </td>
                    <td>{{ $media->file_type ?? '—' }}</td>
                    <td class="whitespace-nowrap">{{ $vm->fmtDate($media->created_at) ?? '—' }}</td>
                    <td>
                        <x-avatar
                    ...
                    <td>
                        <a href="{{ route('tickets.media.view', [$ticket->id, $media->id]) }}" target="_blank" rel="noopener noreferrer"
