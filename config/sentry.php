    ...
    'before_send' => static function (Event $event): ?Event {
        ...
        if ($request !== []) {
            if (isset($request['headers']) && is_array($request['headers'])) {
                $request['headers'] = SentrySanitizer::sanitizeArray($request['headers']);

                $correlationHeader = $request['headers']['X-Correlation-Id']
                    ?? $request['headers']['x-correlation-id']
                    ?? null;
                if (is_array($correlationHeader)) {
                    $correlationHeader = $correlationHeader[0] ?? null;
                }

                if (is_string($correlationHeader) && trim($correlationHeader) !== '') {
                    $tags = $event->getTags();
                    if (! isset($tags['correlation_id'])) {
                        $event->setTag('correlation_id', trim($correlationHeader));
                    }
                }
            }

            if (isset($request['data']) && is_array($request['data'])) {
                $request['data'] = SentrySanitizer::sanitizeArray($request['data']);
            }

            $event->setRequest($request);
        }

        $event->setExtra(SentrySanitizer::sanitizeArray($event->getExtra()));
